...
<c:forEach items="${iList }" var = "inquiry" varStatus="status">
		<div id = "answerbox">
							...
							<hr>
								...
								<div id = "contentTd">내용<br>
								${inquiry.content }</div>
							
							<c:choose>
								<c:when test="${inquiry.s_answer_confirm eq 1 }">
									<hr>
										<div id = "answerTd">완료됨<br>
										<textarea id="answerValue" name="request" >${inquiry.answer }</textarea>
										<input class="btn" type="submit" value="수정" id="updateAjax" data-no="${inquiry.inquiry_id}">
									</div>
								</c:when>
								<c:otherwise>
								<hr>
									...
									<input class="btn" type="submit" value="제출" id="updateAjax" data-no="${inquiry.inquiry_id}"></div>
								</c:otherwise>
...
btn.onclick = function(){
	var answer = $('#answerValue').val();
        ...
        async:false, 
        url:"updateAnswer.do",
        data : {iId:$(this).attr("data-no"), answer:answer},
